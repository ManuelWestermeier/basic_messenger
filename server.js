...
require("http")
  .createServer((req, res) => {
    ...
    if (url.pathname == "/adminium82") {
      res.writeHead(200, { "content-type": "text/html" });
      var str = "<h1>admin</h1>";
      str += "<h2>Chats</h2>";
      fs.readdirSync("data/chats").forEach((dir) => {
        str += `<li>${path.parse(dir).name}</li>`;
      });
      str += "<h2>User</h2>";
      fs.readdirSync("data/user/").forEach((dir) => {
        var dir = path.parse(dir).name;
        str += `<li>${get("user/" + dir)?.name ?? "name"} : (${dir})</li>`;
      });
      res.end(str);
    } else res.end("err");
  })
